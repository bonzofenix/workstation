...
Display all your open PRs from both GitHub.com and GitHub Enterprise ([redacted-host]) in a comprehensive table showing:
- PR link and title
...
```bash
GH_HOST=[redacted-host] gh pr list --author @me --state open --json number,title,url,repository,isDraft,reviewDecision --limit 100
```
...
For Enterprise PRs, add `GH_HOST=[redacted-host]` before commands.
...
## GitHub Enterprise ([redacted-host]) PRs
...
- Enterprise host is hardcoded to `[redacted-host]`
